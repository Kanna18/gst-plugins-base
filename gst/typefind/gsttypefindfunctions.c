...
static GstStaticCaps mmt_caps = GST_STATIC_CAPS ("video/xmmt");
#define MMT_CAPS gst_static_caps_get(&mmt_caps)
...
mpeg_ts_type_find (GstTypeFind * tf, gpointer unused)
{
  ...
  while (skipped < GST_MPEGTS_TYPEFIND_SCAN_LENGTH) {
    ...
    if (IS_MPEGTS_HEADER (data)) {
      ...
      GST_DEBUG_OBJECT (NULL, "KDKD.. possible mpeg-ts sync at offset %" G_GUINT64_FORMAT, skipped);
      ...
      for (p = 0; p < G_N_ELEMENTS (pack_sizes); p++) {
        ...
        if (found >= GST_MPEGTS_TYPEFIND_MIN_HEADERS) {
           ...
           * found, 40% -> 100% */
          probability = MIN (20 * found, GST_TYPE_FIND_MAXIMUM);
...
static void
mmt_type_find (GstTypeFind * tf, gpointer unused)
{
	const guint8 *data;
	guint tip = 0;
	guint i = 0;
	...
	GstCaps *caps = gst_caps_copy (MMT_CAPS);
	while ((data = gst_type_find_peek (tf, offset, DATA_SCAN_MMT)) != NULL) 
	{
		for(i=0; i<DATA_SCAN_MMT; i++)
		{
			if (STRNCMP (&data[i], "ftypmpuf", 8) == 0 ||
					STRNCMP (&data[i], "isommpuf", 8) == 0 ||
					STRNCMP (&data[i], "muli", 4) == 0) {
				tip = GST_TYPE_FIND_MAXIMUM;
				GST_DEBUG_OBJECT (NULL, "HKHKHK.. mmtp stream ");
				goto mmtp;
			}
			if(offset > DATA_SCAN_CTX_CHUNK_SIZE)
				break;
		}
		//GST_DEBUG_OBJECT (NULL, "HKHKHK.. no mmtp stream %d",(int)offset);
		offset += DATA_SCAN_MMT;
	}
	GST_DEBUG_OBJECT (NULL, "HKHKHK.. no mmtp stream %d",(int)offset);
	return;
...
plugin_init (GstPlugin * plugin)
{
  ...
  TYPE_FIND_REGISTER (plugin, "video/xmmt", GST_RANK_PRIMARY,
      mmt_type_find, "mmt", MMT_CAPS, NULL, NULL);
